...
#ifdef WITH_PYTHON2
#define NXWEB_PYTHON2_SETUP(_name, _prefix, ...) extern nxweb_handler nxweb_python2_handler; \
        nxweb_handler _nxweb_ ## _name ## _handler={.name=#_name, .prefix=(_prefix), ## __VA_ARGS__}; \
        _nxweb_register_handler(&_nxweb_ ## _name ## _handler, &nxweb_python2_handler)
#endif

#ifdef WITH_PYTHON3
#define NXWEB_PYTHON3_SETUP(_name, _prefix, ...) extern nxweb_handler nxweb_python3_handler; \
        nxweb_handler _nxweb_ ## _name ## _handler={.name=#_name, .prefix=(_prefix), ## __VA_ARGS__}; \
        _nxweb_register_handler(&_nxweb_ ## _name ## _handler, &nxweb_python3_handler)
#endif
...
static inline uint64_t nxweb_generate_unique_id() {
  ...
  uint64_t uid=(((uint64_t)tdata->thread_num)<<56) // thread_num is highest byte
          | ((((uint64_t)tdata->loop->current_time) & (0xfffff00000<<20))<<16) // time value: changes ~ once per second, repeats after ~ two year's time
...
nxweb_result _nxweb_fc_revalidate(struct nxweb_http_server_connection* conn, nxweb_http_request* req, nxweb_http_response* resp, struct fc_filter_data* fcdata);

nxweb_result _nxweb_fc_store(struct nxweb_http_server_connection* conn, nxweb_http_request* req, nxweb_http_response* resp, struct fc_filter_data* fcdata);
