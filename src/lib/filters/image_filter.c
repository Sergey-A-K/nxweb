...
static int process_cmd(const char* fpath, int doc_root_len, MagickWand* image, nxweb_image_filter_cmd* cmd) {
  ...
  if (cmd->cmd=='s') {
    ...
    if (width>max_width || height>max_height) { // don't enlarge!
      ...
      MagickResizeImage(image, width, height, LanczosFilter);
      dirty=1;
    }
  }
  else if (cmd->cmd=='c') {
    ...
    if (locate_watermark(fpath, doc_root_len, wpath)) {
      ...
      if (wpage_width>wwidth && wpage_height>wheight) { // page defined => watermark file has layout
        if (width<wpage_width || height<wpage_height) { // scale watermark down
          ...
          MagickResizeImage(watermark, wwidth, wheight, LanczosFilter);
        }
...
//     const MagickBooleanType,const ssize_t,const ssize_t),
        ...
        MagickEvaluateImage(watermark, MultiplyEvaluateOperator, 0.5); // AlphaChannel removed
        ...
        image_auto_orient(image);
        MagickCompositeImage(image, watermark, OverCompositeOp, MagickTrue, x, y);
        dirty=1;
      }
      else if (width>=2*wwidth && height>=2*wheight) { // don't watermark images smaller than 2 x watermark
        MagickEvaluateImage(watermark, MultiplyEvaluateOperator, 0.5); // AlphaChannel removed
        image_auto_orient(image);
        MagickCompositeImage(image, watermark, OverCompositeOp, MagickTrue, width-wwidth-10, height-wheight-10);
        dirty=1;
      }
