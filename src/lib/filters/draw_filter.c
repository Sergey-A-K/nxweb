...
//   MagickWand* method=NewMagickWand();
...
//     PixelInterpolateMethod MagickGetImageInterpolateMethod(MagickWand *);
  ...
  for (i=0; i<cmd_len+4; i++) {
    ...
    MagickCompositeImage(bg, rect, XorCompositeOp, 0, 0, UndefinedInterpolatePixel);
  }
  ClearDrawingWand(d_wand);
  ...
  MagickWaveImage(text, 1, 15, UndefinedInterpolatePixel);
  MagickCompositeImage(bg, text, XorCompositeOp, -5, -5, UndefinedInterpolatePixel);
  MagickEvaluateImage(bg, MultiplyEvaluateOperator, 0.5); // AlphaChannel removed , UndefinedInterpolatePixel
