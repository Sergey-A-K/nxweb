...
static void on_config(const nx_json* js) {
  if (nxweb_main_args.python2_root) project_root=nxweb_main_args.python2_root;
  else if (js) {
    const char* v=nx_json_get(js, "project_path")->text_value;
    if (v) project_root=v;
  }
  if (nxweb_main_args.python2_wsgi_app) project_app=nxweb_main_args.python2_wsgi_app;
  else if (js) {
    const char* v=nx_json_get(js, "wsgi_application")->text_value;
    if (v) project_app=v;
  }
  if (nxweb_main_args.python2_virtualenv_path) virtualenv_path=nxweb_main_args.python2_virtualenv_path;
  else if (js) {
    const char* v=nx_json_get(js, "virtualenv_path")->text_value;
    if (v) virtualenv_path=v;
  }
  nxweb_log_error("python2 config: root=%s, app=%s, virtualenv=%s", project_root, project_app, virtualenv_path);
}

static int on_startup() {
  struct stat fi;
  if (!project_app || !*project_app) {
    nxweb_log_error("python2 wsgi app not specified; skipping python2 initialization");
    return 0;
  }
  static const char* prog_name="python2/nxwebpy2.py";
  if (stat(prog_name, &fi)==-1) {

#ifdef NXWEB_LIBDIR
    prog_name=NXWEB_LIBDIR "/nxwebpy2.py";
    if (stat(prog_name, &fi)==-1) {
#endif

      nxweb_log_error("%s is missing; skipping python2 initialization", prog_name);
      return 0;
  ...
  if (!py_module || !PyModule_Check(py_module)) {
    fprintf(stderr, "can't load python2 module %s; check parse errors:\n", MODULE_NAME);
    PyErr_Print();
    exit(0);
  }
...
static void on_shutdown() {
  ...
  nxweb_log_error("shutting down python2");
  PyEval_AcquireLock();
  PyThreadState_Swap(py_main_thread_state);
  Py_XDECREF(py_nxweb_on_request_func);
  Py_XDECREF(py_module);
  Py_Finalize();
  nxweb_log_error("python2 finalized");
}

NXWEB_MODULE(python2, .on_server_startup=on_startup, .on_server_shutdown=on_shutdown, .on_config=on_config);
...
static nxweb_result python_on_post_data(nxweb_http_server_connection* conn, nxweb_http_request* req, nxweb_http_response* resp) {
  ...
  if (!conn->handler->dir) {
    nxweb_log_warning("python2 handler temp upload file dir not set => skipping file buffering for %s", req->uri);
    return NXWEB_NEXT;
  }
...
static nxweb_result python_on_request(nxweb_http_server_connection* conn, nxweb_http_request* req, nxweb_http_response* resp) {
  ...
  nxweb_log_debug("invoke python2");
  ...
  if (py_result && PyTuple_Check(py_result) && PyTuple_Size(py_result)==3) {
    ...
    nxweb_log_error("python2 call failed: %s", PyString_AS_STRING(py_result));
    nxweb_response_printf(resp, "python2 call failed: %H", PyString_AS_STRING(py_result));
  }
  else {
    PyErr_Print();
    nxweb_log_error("python2 call failed");
    nxweb_response_printf(resp, "python2 call failed");
  }
...
NXWEB_DEFINE_HANDLER(python2, .on_select=python_on_select, .on_request=python_on_request,
